...
router.post('/register',(req,res)=>{
  ...
  if(!req.body.email){
    errors.push({message:'Please enter e-mail'});
  }
  if(!req.body.password){
    errors.push({message:'Please enter a password'});
  }
  if(!req.body.passwordConfirm){
    errors.push({message:'This field can not ne blank'});
  }
  if (req.body.password!==req.body.passwordConfirm) {
    errors.push({message:'Passwords field do not match'});
  }
  if (errors.length>0) {
    res.render('home/register',{
      errors:errors,
    ...
    User.findOne({email:req.body.email}).then(user=>{
      if (!user) {
        ...
        bcryptjs.genSalt(10,(err,salt)=>{
          ...
          req.flash('failure_message','Email already exist Please Login');
          res.redirect('/login');
      }
    });
  }
